...
class QuacProcessor(DataProcessor):
    ...
    def __init__(self, tokenizer, conf_or_uncer, max_history):
        ...
        for entry in tqdm(input_data):
            ...
            for paragraph in entry["paragraphs"]:
                ...
                for qa_idx, qa in enumerate(paragraph["qas"]):
                    qas_id = qa["id"]
                    #TODO
                    question_text = self._concat_predicted_history(paragraph["qas"], qa_idx)
    ...
    def _concat_history(self, qas, qa_idx, question_key="question"):
            question_text = ""
    ...
    #TODO
    def _concat_predicted_history(self, qas, qa_idx, question_key="question"):
        question_text = ""
        sep_token = self.sep_token
        if self.tokenizer_name in ["RobertaTokenizer", "RobertaTokenizerFast"]:
            sep_token += self.sep_token
        ...
        for i in range(self.max_history, 0, -1):          
            if qa_idx - i >= 0:
                if self.conf_or_uncer == 'conf':
                    confidence = qas[qa_idx - i]['predicted_answers'][0]['confidence']
                    use_answer = np.random.choice([True, False], p=[confidence, 1-confidence])
                    
                    if use_answer == True:
                        question_text += sep_token + \
                                        qas[qa_idx - i][question_key] + \
                                        sep_token + \
                                        qas[qa_idx - i]['predicted_answers'][0]['text'] #TODO       
                    else:
                        question_text += sep_token + \
                                        qas[qa_idx - i][question_key] 

                elif self.conf_or_uncer == 'uncer':
                    uncertainty = qas[qa_idx - i]['predicted_answers'][0]['uncertainty']
                    use_answer = np.random.choice([True, False], p=[1-uncertainty, uncertainty])
                    ...
                    use_answer = np.random.choice([True, False], p=[confidence_uncertainty, 1-confidence_uncertainty])
        ...
        if qa_idx - i >= 0:
            logger.info("======================")
            logger.info("previous qa: {}".format(qas[qa_idx - i]))
            logger.info("conf_or_uncer: {}".format(self.conf_or_uncer))
            logger.info("use_answer: {}".format(use_answer))
            
            if self.conf_or_uncer == 'conf':
                logger.info("confidence: {}".format(qas[qa_idx - i]['predicted_answers'][0]['confidence']))
                logger.info("[True, False], p=[%f, %f]: ", confidence, 1-confidence)
            
            elif self.conf_or_uncer == 'uncer':
                logger.info("uncertainty: {}".format(qas[qa_idx - i]['predicted_answers'][0]['uncertainty']))
                logger.info("[True, False], p=[%f, %f]: ", 1-uncertainty, uncertainty)

            elif self.conf_or_uncer == 'conf_uncer':
                logger.info("confidence: {}".format(qas[qa_idx - i]['predicted_answers'][0]['confidence']))
                logger.info("uncertainty: {}".format(qas[qa_idx - i]['predicted_answers'][0]['uncertainty']))
                logger.info("1 - uncertainty: %f", 1-uncertainty)
                logger.info("confidence_uncertainty: %f", confidence_uncertainty)
                logger.info("[True, False], p=[%f, %f]: ", confidence_uncertainty, 1-confidence_uncertainty)

...
from quac_processors_step2_train import (
    QuacProcessor,
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss_value = {'orig_loss' : orig_loss.item()}
            loss = args.orig_loss_coeff * orig_loss
            logger.info("orig_loss:{} / Final Loss:{}  ".format(orig_loss, loss))
...
def evaluate(args, model, tokenizer, prefix="", write_predictions=True):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            result = QuacResult(unique_id, start_logits, end_logits, cls_logits)
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        processor = QuacProcessor(tokenizer=tokenizer, conf_or_uncer = args.conf_or_uncer, max_history=args.max_history)
        if evaluate:
...
def main():
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0) and not args.do_valid:

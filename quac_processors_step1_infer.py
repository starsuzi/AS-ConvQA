...
class QuacProcessor(DataProcessor):
    ...
    def __init__(self, tokenizer, max_history):
...
def quac_convert_example_to_features(
    ...
    if not example.is_impossible:
        ...
        if actual_text.find(cleaned_answer_text) == -1:
            ...
            logger.warning("actual_text: '%s'", actual_text)
            logger.warning("cleaned_answer_text: '%s'", cleaned_answer_text)
            logger.warning("example.answer_text: '%s' ", example.answer_text)
            logger.warning("len(actual_text): '%d' ", len(actual_text))
            logger.warning("len(cleaned_answer_text): '%d' ", len(cleaned_answer_text))
            logger.warning("len(example.answer_text): '%d' ", len(example.answer_text))
            logger.warning("=========")
    ...
    #print(tokens[start_position:end_position+1])
    return features
